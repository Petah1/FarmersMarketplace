...
router.post('/', auth, validate([
  ...
  if (endsDate <= new Date()) return err(res, 400, 'ends_at must be in the future', 'validation_error');

  const product = db.prepare('SELECT * FROM products WHERE id = ? AND farmer_id = ?').get(product_id, req.user.id);
  if (!product) return err(res, 404, 'Product not found or not yours', 'not_found');

  const existing = db.prepare(`SELECT id FROM auctions WHERE product_id = ? AND status = 'active'`).get(product_id);
  if (existing) return err(res, 409, 'An active auction already exists for this product', 'conflict');
...
router.post('/:id/bid', auth, validate([
  ...
  if (auction.status !== 'active') return err(res, 400, 'Auction is not active', 'auction_closed');
  if (new Date(auction.ends_at) <= new Date()) return err(res, 400, 'Auction has ended', 'auction_ended');
  ...
  db.prepare('INSERT INTO bids (auction_id, buyer_id, amount) VALUES (?, ?, ?)').run(auction.id, req.user.id, amount);
  db.prepare('UPDATE auctions SET current_bid = ?, highest_bidder_id = ? WHERE id = ?').run(amount, req.user.id, auction.id);
